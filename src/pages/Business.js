...
const Business = () => {
  ...
  if (loading) return <div className="loading">Loading business news...</div>;
  if (error) return <div className="error-message">Error fetching results: {error}</div>;

  return (
    <div>
      <h2 className="page-title">Business News</h2>
      <div className="news-grid">
        {articles.length > 0 ? (
          ...
          <div className="no-results">No business articles found. Please try again later.</div>
        )}
      </div>
    </div>
  );
