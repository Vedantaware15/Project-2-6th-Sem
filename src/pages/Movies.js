...
const Movies = () => {
    ...
    const fetchNews = async () => {
      try {
        const res = await axios.get(
          `https://newsapi.org/v2/everything?q=movie&apiKey=${process.env.REACT_APP_NEWS_API_KEY}`
        );
        setArticles(res.data.articles);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching movie news:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="loading">Loading movie news...</div>;
  if (error) return <div className="error-message">Error: {error}</div>;

  return (
    <div>
      <h2 className="page-title">Movie News</h2>
      <div className="news-grid">
        {articles.length > 0 ? (
          ...
          <div className="error-message">
            No movie articles found. Please try again later.
          </div>
        )}
      </div>
    </div>
