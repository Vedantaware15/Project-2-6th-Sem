...
const Entertainment = () => {
    ...
    const fetchNews = async () => {
      try {
        const res = await axios.get(
          `https://newsapi.org/v2/top-headlines?country=us&category=entertainment&apiKey=${process.env.REACT_APP_NEWS_API_KEY}`
        );
  ...
  if (loading) return <div className="loading">Loading entertainment news...</div>;
  if (error) return <div className="error-message">Error: {error}</div>;

  return (
    <div>
      <h2 className="page-title">Entertainment News</h2>
      <div className="news-grid">
        {articles.length > 0 ? (
          ...
          <div className="error-message">
            No entertainment articles found. Please try again later.
          </div>
        )}
      </div>
    </div>
